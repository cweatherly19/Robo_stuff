...
###########################################################################
######## The below definitions are here only to prove a point
###########################################################################
...
##########################################################################################
##### This would be Voss's stuff, but this code is in here just to show what would happen
##########################################################################################

        elif key.upper() == 'I': #drive forwards
            forwards()
        elif key.upper() == 'K': #drive backwards
            backwards()

        elif key.upper() == 'J': #turn left
            turn_left()
        elif key.upper() == 'L': #turn right
            turn_right()

        global start #send the release signal to the key_release function
        start = time.time()
...
def key_release():
    while True:
        if time.time() - start > 0.55: #takes 0.55 seconds to register key release
            stop_all()
...
threading.Thread(target = key_release, name = 'key_release').start() #This won't be in the final code
